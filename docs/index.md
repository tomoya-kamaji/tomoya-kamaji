...
エンジニアとして 7 年の経験。TypeScript、Golang、Ruby on Rails を用いた Web アプリケーション開発に従事
秒間 6,000 リクエストの高負荷システム運用実績を持つ。

DDD 設計でのリアーキテクチャ、アプリケーション層での証明書検証実装など、
ゼロイチでの技術基盤構築を得意とする。現在は医療 SaaS のテックリードとして、設計支援、開発プロセス改善、オブザーバビリティ強化を推進。
電子カルテ連携 API のセキュリティ基盤構築では、設計から実装、プロジェクト管理まで
一貫して担当し、本番運用 1 年間でインシデント 0 。

また、プロジェクトマネジメントにも注力し、コーディング規約、設計レビュー、
設計プロセスの導入、ドキュメント管理、
タスク管理の標準化により、チーム全体の生産性向上に貢献
...
TypeScript、Golang、Ruby on Rails を用いたアプリケーション開発に従事してきました。
要件・デザインからシステムを設計し、適切な粒度のタスクに分解することができます。

### 開発プロセス・チーム標準化

チームに適度な規律を持ち込み、デリバリーを加速させるマネジメントスキル

- Design Docs の運用とフォーマット化
- コーディング規約、設計レビュープロセスの導入
- Cursor ルールの策定など、AI 時代の開発環境整備
- プロジェクト管理テンプレート構築
- コードレビューのチーム運用ルール策定
- ドキュメント管理、タスク管理の標準化
...
秒間 6,000 リクエスト発生する広告配信サービスの運用経験があるため、
パフォーマンスチューニングやサーバー負荷削減のノウハウを持っています。

### インフラ構築

シンプルなアプリケーションであれば、IaC（AWS CDK、Terraform）を用いてインフラを構築することもできます
...
初めての事例として、電子同意書サービスと電子カルテをつなぐ連携 API のセキュリティ設計と実装をリード
API キー＋ クライアント証明書の多層認証を実装 → [詳細](jobs/con/api-cooperation.md)

医療向け説明動画配信機能の再設計と基盤刷新について、プロジェクトをリードし、設計支援とデータ構造整理を行い、期日内リリースに貢献
